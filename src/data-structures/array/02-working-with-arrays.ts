...
const createArray = (size: number, initialValue: number | undefined = undefined): number[] =>
...
const doWhileEmulation = (arr: number[], action: (value: number, index: number) => void) => {
    let i = 0;
    do {
        action(arr[i], i);
        i++;
...
doWhileEmulation(arr3, (value, index) => console.log(`Index ${index}: ${value}`));
